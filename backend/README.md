...
- **Submissions**: One submission per (task, student)—DB unique constraint + app check (idempotency). QUIZ auto-graded (correct → APPROVED, wrong → REJECTED); AUDIO/PHOTO start as PENDING, teacher reviews via PATCH. **Sprint 4 rules and contract:** see [docs/SPRINT4_ARCHITECTURE.md](./docs/SPRINT4_ARCHITECTURE.md) (Task vs Submission, answer schemas per task type, teacher/student flows, attempt policy).
